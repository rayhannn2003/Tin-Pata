...
import { useAllBookmarks } from '@/features/bookmarks/useAllBookmarks';
...
export default function BookmarksScreen() {
  ...
  const { bookmarks, loading } = useAllBookmarks(search, bookId);
  const { book } = useBook(bookId);

  const title = useMemo(() => {
    if (bookId && book) {
      return t('annotations.bookmarksForBook', { title: book.title });
    }
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={styles.content}>
        <TextInput
          ...
          placeholder={t('annotations.searchBookmarks')}
        ...
        {loading ? (
          ...
          <AppEmptyState
            title={t('annotations.noBookmarksFound')}
            message={
              search.trim()
                ? t('annotations.noBookmarksMatch')
